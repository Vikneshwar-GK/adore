...
- **`deploy_repair(repair_id)` will live in `repair_engine.py`** — writes new SQL to disk, runs `dbt run --select {model}+` via subprocess to rebuild the model and all downstream. Built in Task 15 alongside the approval UI where it can be tested end-to-end.
